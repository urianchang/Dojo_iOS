...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var curVelocity : Int = 100
    var curVelIdx : Float = 3
    
    weak var gameviewcontroller: GameViewController?
    
    var testArr1 = [[false, false, false, false, false, false, false],
...
//        keysY = [400, 372, 344, 316, 288, 260, 232, 204, 176, 148, 120, 92, 62]
        ...
        for i in 0...7 {
            if velocities[i] == curVelocity {
                curVelIdx = Float(i)
            }
        }
        tempoSlider!.value = curVelIdx
        hud.changetempo(tempo: curVelocity)
...
//        self.camera!.removeFromParent()
        ...
        curVelocity = 100
        curVelIdx = 3
        testArr1 = [[false, false, false, false, false, false, false],
                    [false, false, false, false, false, false, false],
                    [false, false, false, false, false, false, false],
                    [false, false, false, false, false, false, false],
                    [false, false, false, false, false, false, false],
        ...
        if let scene = GKScene(fileNamed: "GameScene") {
            if let sceneNode = scene.rootNode as! GameScene? {
                sceneNode.scaleMode = .aspectFill
                    sceneNode.gameviewcontroller = self.gameviewcontroller
                    sceneNode.indexPath = self.indexPath
                    sceneNode.curVelocity = 100
                    sceneNode.curVelIdx = 3
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in (touches) {
            ...
            if let gameSprite = nodeTouched as? GameSprite {
                if let strArr = nodeTouched.name?.components(separatedBy: "_") {
                    testArr1[Int(strArr[0])!][Int(strArr[1])!] = !(testArr1[Int(strArr[0])!][Int(strArr[1])!])
                }
                gameSprite.onTap()
            }
...
//                self.scene?.removeFromParent()
//                tempoSlider!.removeFromSuperview()
                let array = testArr1 as NSObject
                self.gameviewcontroller?.itemsaved(title: "Chorus", instrument: "piano", tempo: Int16(curVelocity), song: array, indexpath: indexPath)
                self.gameviewcontroller?.performSegue(withIdentifier: "unwind", sender: indexPath)
                print("saved")
            }
        }
    }
